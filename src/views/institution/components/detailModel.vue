<script setup lang="ts">
  ...
  watch(
    ...
    (value) => {
      ...
      if (value) {
        getDictionaryTypeSelector('sbgjcl').then((res: any) => {
  ...
  const emits = defineEmits(['closeModel', 'success']);
  const openModal = ref<boolean>(false);
  const remark = ref<string>('');
  const radioList = ref<any>([]);
  const handleSubmit = async () => {
    if (radioType.value) {
      let res: any = await alarmSingle({
      ...
      <div class="ml-34px mr-34px main-container">
        <div class="top-container">
          <div class="flex mb-10px">
            ...
            <span :class="statusClass[gjDetailData.status]">{{ gjDetailData.statusText }}</span>
          </div>
